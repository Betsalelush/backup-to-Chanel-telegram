...
class מעביר_טלגרם:
    ...
    async def התחל_העברה(self):
        ...
        print("\n=== מעביר הודעות טלגרם (גרסה מתקדמת) ===\n")
        ...
        print("3. התחל ממספר הודעה מסוים")
        ...
        if בחירה == '2':
            ...
            while True:
                try:
                    מספר_הודעה = input("הזן את מספר ההודעה להתחיל ממנה: ").strip()
                    מספר_הודעה = int(מספר_הודעה)
                    התקדמות = {"הודעה_אחרונה": מספר_הודעה, "סך_הועברו": 0}
                    print(f"✅ יתחיל מהודעה מספר {מספר_הודעה}")
                    break
                except ValueError:
        ...
        try:
            ...
            logger.info("\n⚠️ העברה הופסקה על ידי המשתמש.")
        except Exception as e:
            logger.error(f"שגיאה כללית: {e}")
        
        finally:
            self.שמור_התקדמות(התקדמות)
            logger.info(f"\n✅ סבב הושלם! סה\"כ הועברו {התקדמות['סך_הועברו']} הודעות.")
